...
const validateEmail = () => {
  const pattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  emailValid.value = !!email.value.trim() && pattern.test(email.value)
}
...
const onSubmit = async () => {
  ...
  try {
    await $fetch('/api/rails-waitlist', {
      method: 'POST',
...
<template>
  <div>
    <form v-show="!formSubmitted" novalidate @submit.prevent="onSubmit" class="space-y-4 p-6 bg-neutral-50 rounded-lg">
      <h2 class="mt-0">Join the Rails Rudiment waitlist</h2>
      <FormField v-model="email" inputId="email" name="email" label="Email" type="email" :isValid="emailValid"
